...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex align-items-center gap-10 justify-content-center">
                                                ...
                                                <a href="{{ route('jobOffer.details', $job_offer->id) }}"
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-bs-target="#editJobOfferModal{{ $job_offer->id }}">
                                                    <iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
                                                </button>

                                                <!-- Bouton SUPPRIMER -->
                                                <form method="POST" action="{{ route('career.destroy', $job_offer->id) }}"
    ...
    @foreach ($job_offers as $job_offer)
        ...
        <div class="modal fade" id="editJobOfferModal{{ $job_offer->id }}" tabindex="-1"
            aria-labelledby="editJobOfferModalLabel{{ $job_offer->id }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    <form method="POST" action="{{ route('career.update', $job_offer->id) }}">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title" id="editJobOfferModalLabel{{ $job_offer->id }}">Modifier l'offre :
                                {{ $job_offer->title }}</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="title" id="title{{ $job_offer->id }}"
                                    class="form-control @error('title', 'update-' . $job_offer->id) is-invalid @enderror"
                                    value="{{ old('title', $job_offer->title) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="type" id="type{{ $job_offer->id }}"
                                    class="form-select @error('type', 'update-' . $job_offer->id) is-invalid @enderror"
                                    required>
                                    <option value="">-- Sélectionner un type --</option>
                                    <option value="stage"
                                        {{ old('type', $job_offer->type) == 'stage' ? 'selected' : '' }}>Stage</option>
                                    <option value="emploi"
                                        {{ old('type', $job_offer->type) == 'emploi' ? 'selected' : '' }}>Emploi</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="status{{ $job_offer->id }}"
                                    class="form-select @error('status', 'update-' . $job_offer->id) is-invalid @enderror"
                                    required>
                                    <option value="">-- Sélectionner un statut --</option>
                                    <option value="open"
                                        {{ old('status', $job_offer->status) == 'open' ? 'selected' : '' }}>Ouvert</option>
                                    <option value="closed"
                                        {{ old('status', $job_offer->status) == 'closed' ? 'selected' : '' }}>Fermé
                                    </option>
                                </select>
                            ...
                            <div class="mb-3">
                                <label for="bref_description{{ $job_offer->id }}" class="form-label">Brève description
                                    *</label>
                                <textarea name="bref_description" id="bref_description{{ $job_offer->id }}"
                                    class="form-control @error('bref_description', 'update-' . $job_offer->id) is-invalid @enderror" rows="3"
                                    required>{{ old('bref_description', $job_offer->bref_description) }}</textarea>
                                @error('bref_description', 'update-' . $job_offer->id)
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="description{{ $job_offer->id }}"
                                    class="form-control summernote @error('description', 'update-' . $job_offer->id) is-invalid @enderror"
                                    rows="5" required>{{ old('description', $job_offer->description) }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if (session('edit_job_offer_id'))
                ...
                'editJobOfferModal{{ session('edit_job_offer_id') }}');
                if (modalElement) {
