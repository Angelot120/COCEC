...
<main class="dashboard-main">
    ...
    <div class="dashboard-main-body">
        ...
        <div class="card h-100 p-0 radius-12">
            <div class="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between">
                <form action="{{ route('agency.index') }}" method="GET" class="d-flex align-items-center flex-wrap gap-3">
                ...
                <a href="{{ route('agency.create') }}" class="btn btn-danger text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <span class="bg-success-focus text-success-600 border border-success-main px-24 py-4 radius-4 fw-medium text-sm">Ouverte</span>
                                    @elseif ($agency->status === 'Close')
                                    <span class="bg-neutral-200 text-neutral-600 border border-neutral-400 px-24 py-4 radius-4 fw-medium text-sm">Fermée</span>
                                    @else
                                    <span class="bg-neutral-200 text-neutral-600 border border-neutral-400 px-24 py-4 radius-4 fw-medium text-sm">Non défini</span>
                                    @endif
                                </td>
                                <td class="text-center">
                                    <div class="d-flex align-items-center gap-10 justify-content-center">
                                        <!-- Bouton VOIR -->
                                        <button type="button" class="bg-info-focus text-info-600 bg-hover-info-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle" data-bs-toggle="modal" data-bs-target="#viewAgencyModal{{ $agency->id }}">
                                            <iconify-icon icon="lucide:eye" class="menu-icon"></iconify-icon>
                                        </button>

                                        <!-- Bouton EDITION -->
                                        <button type="button" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle" data-bs-toggle="modal" data-bs-target="#editAgencyModal{{ $agency->id }}">
                                            <iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
                                        </button>

                                        <!-- Bouton SUPPRIMER -->
                                        <form method="POST" action="{{ route('agency.destroy', $agency->id) }}" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="modal fade" id="viewAgencyModal{{ $agency->id }}" tabindex="-1" aria-labelledby="viewAgencyModalLabel{{ $agency->id }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="viewAgencyModalLabel{{ $agency->id }}">Détails de l'agence : {{ $agency->name }}</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div id="view-map-{{ $agency->id }}" class="modal-map" style="height: 250px;"></div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editAgencyModal{{ $agency->id }}" tabindex="-1" aria-labelledby="editAgencyModalLabel{{ $agency->id }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form method="POST" action="{{ route('agency.update', $agency->id) }}" class="modal-content">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title" id="editAgencyModalLabel{{ $agency->id }}">Modifier l'agence : {{ $agency->name }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" id="name{{ $agency->id }}" class="form-control @error('name', 'update-'.$agency->id) is-invalid @enderror" value="{{ old('name', $agency->name) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="any" name="latitude" id="latitude{{ $agency->id }}" class="form-control @error('latitude', 'update-'.$agency->id) is-invalid @enderror" value="{{ old('latitude', $agency->latitude) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="any" name="longitude" id="longitude{{ $agency->id }}" class="form-control @error('longitude', 'update-'.$agency->id) is-invalid @enderror" value="{{ old('longitude', $agency->longitude) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="address" id="address{{ $agency->id }}" class="form-control @error('address', 'update-'.$agency->id) is-invalid @enderror" rows="3" required>{{ old('address', $agency->address) }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="phone" id="phone{{ $agency->id }}" class="form-control @error('phone', 'update-'.$agency->id) is-invalid @enderror" value="{{ old('phone', $agency->phone) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="status{{ $agency->id }}" class="form-select @error('status', 'update-'.$agency->id) is-invalid @enderror">
                                    <option value="" {{ old('status', $agency->status) == null ? 'selected' : '' }}>Non défini</option>
                                    <option value="Open" {{ old('status', $agency->status) == 'Open' ? 'selected' : '' }}>Ouverte</option>
                                    <option value="Close" {{ old('status', $agency->status) == 'Close' ? 'selected' : '' }}>Fermée</option>
                                </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <div id="edit-map-{{ $agency->id }}" class="modal-map" style="height: 300px;"></div>
                            <small class="text-muted">Cliquez sur la carte pour changer la position du marqueur</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                ...
                agencies.forEach(function(agency) {
                    ...
                    if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
                        let marker = L.marker([lat, lng])
                            .addTo(mainMap)
                            .bindPopup('<b>' + (agency.name || 'Nom non défini') + '</b><br>' + (agency.address || 'Adresse non définie') + '<br>Tél: ' + (agency.phone || 'Non défini'));
