...
@section('css')
{{-- CSS requis pour la carte interactive Leaflet --}}
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" integrity="sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=" crossorigin="" />

<style>
    /* === STYLE "PRESTIGE" DE LA PAGE AGENCES (Marqueurs Classiques) === */
    ...
    .agency-search-bar input::placeholder { color: #A0AEC0; }
    ...
    #agency-map { width: 100%; height: 100%; }

    /* --- En-tête de la Liste et Compteur --- */
    .agency-list-header {
        margin-bottom: 25px;
        padding-bottom: 15px;
    ...
    .agency-list-container::-webkit-scrollbar { width: 6px; }
    .agency-list-container::-webkit-scrollbar-track { background: #EDF2F7; }
    .agency-list-container::-webkit-scrollbar-thumb { background-color: #CBD5E0; border-radius: 6px; }
    ...
    .agency-card .card-content { padding: 25px; }
    ...
    .status-dot.open { background-color: #38A169; }
    .status-dot.closed { background-color: #E53E3E; }
    .agency-status { font-weight: 500; font-size: 0.9rem; }

    .agency-info { list-style: none; padding: 0; margin: 0 0 20px 0; }
    .agency-info li { display: flex; align-items: flex-start; margin-bottom: 12px; color: var(--text-color); }
    .agency-info .info-icon { color: var(--primary-color); font-size: 1rem; width: 20px; text-align: center; margin-right: 15px; margin-top: 2px;}

    .agency-actions { display: flex; gap: 10px; }
    .btn-action {
        flex: 1; padding: 10px 15px; border-radius: 8px; font-weight: 600; text-decoration: none;
        text-align: center; transition: all 0.2s ease; border: 2px solid transparent;
    }
    .btn-route { background-color: var(--primary-color); color: #FFFFFF; }
    .btn-route:hover { background-color: #c01e13; }
    .btn-call { background-color: transparent; color: var(--dark-charcoal); border-color: var(--border-color); }
    .btn-call:hover { background-color: var(--light-gray-bg); }
    
    .no-results-message {
        text-align: center; padding: 40px 20px; background-color: #fff; border-radius: 12px; border: 1px solid var(--border-color);
    }
    .no-results-message i { font-size: 2.5rem; color: #CBD5E0; margin-bottom: 15px; }
    .no-results-message p { font-size: 1.1rem; color: var(--text-color); font-weight: 500; margin: 0; }

    /* --- Responsive --- */
    @media (max-width: 991px) {
        .agency-page-layout { grid-template-columns: 1fr; }
        .agency-map-container { position: relative; top: 0; height: 50vh; }
        .agency-list-container { height: auto; overflow-y: visible; }
    }
</style>
@endsection
        ...
        <div class="agency-page-layout">
            ...
            <div>
                ...
                <div class="agency-list-container" id="agency-list">
                    {{-- Les agences et le message "aucun résultat" seront injectés ici --}}
                </div>
            </div>
        </div>
    </div>
...
@section('js')
{{-- JS requis pour la carte interactive Leaflet --}}
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const agencies = [{
                id: 1, name: "Siège Social", lat: 6.13845, lng: 1.21342,
                address: "Boulvard Jean-Paul II, face station d'essence, Lomé",
                phone: "91 84 84 81", status: "open"
            },
            {
                id: 2, name: "Agence de l'Étoile Rouge", lat: 6.15581, lng: 1.20911,
                address: "Non loin de la Douane, Lomé",
                phone: "91 84 84 82", status: "open"
            },
            {
                id: 3, name: "Agence d'Adidogomé", lat: 6.17642, lng: 1.15785,
                address: "Route de Kpalimé, Von de la Radio Merveille",
                phone: "91 84 84 83", status: "closed"
            },
            {
                id: 4, name: "Agence d'Agoè", lat: 6.21633, lng: 1.20340,
                address: "Face église catholique Christ-Roi, Agoè",
                phone: "91 84 84 84", status: "open"
            },
            {
                id: 5, name: "Agence de Kpalimé", lat: 6.89956, lng: 0.63001,
                address: "Route de Lomé, face station d'essence",
                phone: "91 84 84 85", status: "open"
            }
        ];
        ...
        // Icône de marqueur personnalisée (SVG)
        ...
        function renderListAndMarkers(filteredAgencies) {
            ...
            filteredAgencies.forEach(agency => {
                    ...
                    <div class="card-content">
                        <div class="card-header">
                            ...
                            <span class="agency-status">
                                <span class="status-dot ${agency.status}"></span>
                        ...
                        <div class="agency-actions">
                            <a href="https://www.google.com/maps/dir/?api=1&destination=${agency.lat},${agency.lng}" target="_blank" class="btn-action btn-route"><i class="fas fa-directions"></i> Itinéraire</a>
                ...
                const marker = L.marker([agency.lat, agency.lng], { icon: customIcon, riseOnHover: true })
        ...
        function handleInteraction(id, source = 'card') {
            ...
            map.flyTo([agency.lat, agency.lng], 15);
        ...
        function attachCardEventListeners() {
            document.querySelectorAll('.agency-card').forEach(card => {
                card.addEventListener('click', function() { handleInteraction(parseInt(this.dataset.id)); });
            });
        }
